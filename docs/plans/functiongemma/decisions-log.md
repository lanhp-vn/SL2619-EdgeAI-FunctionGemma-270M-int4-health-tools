...
| 2026-05-01 | Distil platform DOES accept `functiongemma-270m-it` for `multi-turn-tool-calling-closed-book` | The local catalog snapshot was stale. Distil's blog "Making FunctionGemma Work" and the public HF model `distil-labs/distil-home-assistant-functiongemma` both train this exact combo. Verified empirically by `distil model upload-data --dry-run` (free, no credit cost). | DONE — `docs/bench-notes/functiongemma/2026-05-01_functiongemma-distil-labs-data-plan.md` |
